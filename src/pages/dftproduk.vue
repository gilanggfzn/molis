      ...
      <div class="border rounded-full">
        <div class="relative overflow-x-auto sm:rounded-lg">
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              <tr
                v-for="(item, index) in filterProduk"
                :key="index"
              ...
              >
                ...
                <td class="px-6 py-4">
                  <a class="text-blue-500" target="_black" :href="item.linkWebsiteProduk"
...
export default {
  ...
  methods: {
    ...
    async deactiveProduk(id) {
      try {
        const response = await axios.put(
          ...
          `https://molis-production.up.railway.app/api/produk/toggleStatus/${id}`
        );
    ...
    async getFindByName() {
      try {
        const response = await axios.get(
          `https://molis-production.up.railway.app/api/produk/search?searchTerm=${this.search}`
        );
        this.Produk = response.data;
      } catch (error) {
        console.error("Failed to fetch Produk:", error);
  ...
  computed: {
    ...
    filterProduk() {
      ...
      return this.Produk.slice(start, end);
    },
  },
};
</script>
